...
class rSigRnx():
    ...
    def frequency(self, k=None):
        ...
        if self.sys == uGNSS.GPS:
            ...
            k_val = k if k is not None else 0
            if int(self.sig / 100) == 1:
                return rCST.FREQ_R1 + k_val * rCST.FREQ_R1k
            elif int(self.sig / 100) == 2:
                return rCST.FREQ_R2 + k_val * rCST.FREQ_R2k
...
def satazel(pos, e):
    ...
    enu = ecef2enu(pos, e)
    az = atan2(enu[1], enu[0])
    z = atan2(np.sqrt(enu[1]**2+enu[0]**2), enu[2])
    if z > 0:
        el = z - rCST.PI/2
    elif z < 0:
        z = z + rCST.PI
        ...
        print(f"satazel: zenith angle error, z={z}, enu={enu}, pos={pos}, e={e}, az={az}")
    #el = asin(enu[2])
    return az, el
